 ...
 */
final class Version20211018133936 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__task AS SELECT id, todo_id, user_id, taskname FROM task');
        $this->addSql('DROP TABLE task');
        $this->addSql('CREATE TABLE task (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, todo_id INTEGER NOT NULL, user_id INTEGER NOT NULL, taskname VARCHAR(255) NOT NULL COLLATE BINARY, status VARCHAR(255) DEFAULT NULL, CONSTRAINT FK_527EDB25EA1EBC33 FOREIGN KEY (todo_id) REFERENCES todo (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_527EDB25A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO task (id, todo_id, user_id, taskname) SELECT id, todo_id, user_id, taskname FROM __temp__task');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__task AS SELECT id, todo_id, user_id, taskname FROM task');
        $this->addSql('DROP TABLE task');
        $this->addSql('CREATE TABLE task (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, todo_id INTEGER NOT NULL, user_id INTEGER NOT NULL, taskname VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO task (id, todo_id, user_id, taskname) SELECT id, todo_id, user_id, taskname FROM __temp__task');
        $this->addSql('DROP TABLE __temp__task');
        $this->addSql('CREATE INDEX IDX_527EDB25EA1EBC33 ON task (todo_id)');
        $this->addSql('CREATE INDEX IDX_527EDB25A76ED395 ON task (user_id)');
